...
class AinasciiA2bHex():
    ...
    def terminal_vehicle_login(self,vin):
        ...
        # UAT环境ISUTEST0154376298
        # binascii_data = '232301FE4A4C4630303030303030303030303030390100361308150E3314000132393239383937363534343531313131303030310118535657504541314137375631323137375630303030313231BF'
        # 新能源798325报文
    ...
    def terminal_vehicle_upload_realtime(self,vin,Longitude,Latitude,mileage):
        ...
        binascii_data = binascii_data.replace('070000000000', '070300000000')  # 设置报警
    ...
    def terminal_vehicle__realtime_info(self,vin):
        ...
        binascii_data = '2323C5FE495355544553543534363837323933313001005500531407080E2438271102000427120111271302001527140F0014001400140014000000140014002715063C3C3C3C3C3C27160200D7271701022718040000E958271904000155D1271A01034E2101024E2202012CA1'
    ...
    def terminal_vehicle_upload_realtime_flameout(self,vin,Longitude,Latitude,mileage):
        ...
        binascii_data = ''   #默认报文
        # 有错误的报文无效车速、里程
        ...
        # binascii_data = binascii_data.replace('070000000000', '070300000080')  # 设置报警
        binascii_data = binascii_data.replace('070000000000', '070300000000')
        binascii_data = binascii_data.replace(binascii_data[72:80], mileage)  # 替换里程
        data = binascii_data[:-2]
        byte1 = int(str(data[0:2]), 16)
        for i in range(len(data) - 2):   #XOR异或运算得出BCC
...
if __name__ == '__main__':
    ...
    for i in range(len(vin)):
        ...
        time.sleep(3)
        print("发送车辆熄火")
        # tcpCliSock.send(lib.terminal_vehicle_upload_realtime_2(vin[i], Longitude[x], Latitude[x],mileage))  # 补发
        time.sleep(1)
